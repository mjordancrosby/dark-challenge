int getMinDenoms(List<int> coins, int amount) {
  ...
  coins.forEach((coin) => {
    if (coin <= 0) {
      throw ArgumentError("${coin} invalided value. Coins must contain postive integers only")
    }
  });
  ...
  for (int i = 1; i <= amount; i++) {
    ...
    for (int coin in coins) {
      if (i - coin >= 0) {
        subAmmount[i] = subAmmount[i] <= subAmmount[i - coin] + 1 ? subAmmount[i] : subAmmount[i - coin] + 1;
      }
    }
  }
